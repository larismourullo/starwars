...
export class VehiclesComponent implements OnInit {
  ...
  getRandomVehicle() {
    ...
    this.dataService.getPageData(this.category, 1).subscribe(data => {
      ...
      let positionAtPage = randomIndex - ((Math.ceil(randomVehiclePosition / vehiclesPerPage) - 1) * vehiclesPerPage);
      let randomVehiclePage = Math.ceil(randomVehiclePosition / vehiclesPerPage);

      this.dataService.getPageData(this.category, randomVehiclePage).subscribe(data => {
